...
const TABS: TabItem[] = [
  ...
  { id: "prompts", label: "AI Questions", children: <p>AI Questions panel</p> },
];
...
describe("Tabs", () => {
  ...
  it("renders a tab button for each tab", () => {
    ...
    expect(screen.getByRole("tab", { name: "AI Questions" })).toBeInTheDocument();
  });
  ...
  it("honors defaultTab when provided", () => {
    ...
    expect(screen.getByRole("tab", { name: "AI Questions" })).toHaveAttribute(
      "aria-selected",
      "true",
    );
  });
  ...
  it("renders only the active panel content", () => {
    ...
    expect(screen.queryByText("AI Questions panel")).not.toBeInTheDocument();
  });
  ...
  it("preserves other URL search params when the tab changes", async () => {
    ...
    render(<Tabs tabs={TABS} />);
    await userEvent.click(screen.getByRole("tab", { name: "AI Questions" }));
