...
import { useTrackerDepth } from "@/features/reports/hooks/useTrackerDepth";
...
export function TrackerDashboardV2Screen({ trackerId }: TrackerDashboardV2ScreenProps) {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Phase 4 v2 Slice C: platform/topic/activity + recent chats. Same
          pattern — separate fetch, separate failure boundary. */}
      <DepthSections trackerId={trackerId} days={days} />
    </div>
  );
}
...
// ---------------------------------------------------------------------------
// Slice C sections — fetched from /dashboard/depth
// ---------------------------------------------------------------------------

interface DepthSectionsProps {
  trackerId: string;
  days: number;
}

function DepthSections({ trackerId, days }: DepthSectionsProps) {
  const { data, isLoading, isError } = useTrackerDepth(trackerId, days);
  const [selectedChat, setSelectedChat] = useState<RecentChatDto | null>(null);
...
function MentionsByPlatformCard({ rows }: { rows: readonly PlatformMentionDto[] }) {
  const copy = REPORTS_COPY.dashboard.platforms;
...
function SentimentDistributionCard({ slices }: { slices: readonly SentimentSliceDto[] }) {
  const copy = REPORTS_COPY.dashboard.sentiment;
  const data: DonutChartDatum[] = useMemo(
    () =>
      slices.map((s) => ({
        id: s.sentiment,
        label: s.sentiment,
...
function ActivityHeatmapCard({ heatmap }: { heatmap: HeatmapDto }) {
  const copy = REPORTS_COPY.dashboard.activity;
...
function TopicHeatmapCard({ heatmap }: { heatmap: HeatmapDto }) {
  const copy = REPORTS_COPY.dashboard.topicCoverage;
  const data: HeatmapData = useMemo(
    () => ({
      rows: [...heatmap.rows],
      cols: [...heatmap.columns],
...
interface RecentChatsCardProps {
  chats: readonly RecentChatDto[];
  onSelect: (chat: RecentChatDto) => void;
}

function RecentChatsCard({ chats, onSelect }: RecentChatsCardProps) {
  const copy = REPORTS_COPY.dashboard.recentChats;
  return (
    <Card>
      <CardHeader>
        <CardTitle>{copy.title}</CardTitle>
...
interface RecentChatDrawerProps {
  chat: RecentChatDto | null;
  onClose: () => void;
}

function RecentChatDrawer({ chat, onClose }: RecentChatDrawerProps) {
  const copy = REPORTS_COPY.dashboard.recentChats.drawer;
  if (!chat) return null;
  return (
    <div
      role="dialog"
