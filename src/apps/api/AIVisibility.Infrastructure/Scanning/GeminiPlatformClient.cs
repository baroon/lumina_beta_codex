...
public class GeminiPlatformClient : IPlatformClient
{
    ...
    {
        var env = await _gemini.ChatCompletionAsync(ScanSystemPrompt.Value, prompt, 1024, 0.7, cancellationToken);
        return PlatformClientEnvelope.BuildScanAnswer(env, "Gemini");
    }
}
