...
public class GrokPlatformClient : IPlatformClient
{
    ...
    {
        var env = await _grok.ChatCompletionAsync(ScanSystemPrompt.Value, prompt, 1024, 0.7, cancellationToken);
        return PlatformClientEnvelope.BuildScanAnswer(env, "Grok");
    }
}
