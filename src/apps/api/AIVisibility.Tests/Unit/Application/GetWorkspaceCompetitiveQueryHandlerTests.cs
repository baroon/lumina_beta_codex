...
public class GetWorkspaceCompetitiveQueryHandlerTests
{
    ...
    public async Task ReturnsEmpty_WhenWorkspaceHasNoBrands()
    {
        ...
        var result = await sut.Handle(new GetWorkspaceCompetitiveQuery(DateTime.UtcNow.AddDays(-30), null, null, null, null, null), CancellationToken.None);
    ...
    public async Task LensCodeFilter_RestrictsTheAnswerSet_AndAllDerivedSections()
    {
        ...
        var emptyFilter = await sut.Handle(
            new GetWorkspaceCompetitiveQuery(
                DateTime.UtcNow.AddDays(-30), null, new[] { "nonexistent-lens" }, null, null, null),
        ...
        var matched = await sut.Handle(
            new GetWorkspaceCompetitiveQuery(
                DateTime.UtcNow.AddDays(-30), null, new[] { "x" }, null, null, null),
            CancellationToken.None);
