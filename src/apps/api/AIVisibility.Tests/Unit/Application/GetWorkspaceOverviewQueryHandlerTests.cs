...
public class GetWorkspaceOverviewQueryHandlerTests
{
    ...
    public async Task ReturnsEmpty_WhenWorkspaceHasNoBrands()
    {
        ...
        var result = await sut.Handle(new GetWorkspaceOverviewQuery(DateTime.UtcNow.AddDays(-30), null, null, null), CancellationToken.None);
    ...
    public async Task TopEntities_SentimentDelta_IsNull_WhenOnlyOneScanInWindow()
    {
        ...
        var result = await sut.Handle(new GetWorkspaceOverviewQuery(DateTime.UtcNow.AddDays(-5), null, null, null), CancellationToken.None);
